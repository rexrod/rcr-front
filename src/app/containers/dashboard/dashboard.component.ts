...
import { NoticeControllerService } from 'app/module/notices';
...
export class DashboardComponent implements OnInit {
  ...
  advancedPieChartOptions: AdvancedPieChartWidgetOptions = {
    ...
    subTitle: 'Andamento Anual das Ordens de Serviço\n'
  };
  ...
  noticeStyles(color) {
    const styles = {
      'background-color': this.LightenDarkenColor(color, 180),
      'border-color': this.LightenDarkenColor(color, -50),
      'color': this.LightenDarkenColor(color, -150),
      'margin-bottom': '10px',
    };
    return styles;
  }
  ...
  ngOnInit() {
    ...
    const demoPages = [];
    const demoPagesPossibleValues = ['/components', '/tables/all-in-one-table', '/apps/inbox', '/apps/chat', '/dashboard', '/users', '/register', '/apps/calendar', '/forms/form-elements'];
    for (let i = 0; i < 3; i++) {
